...
const save = async (req, res) => {
  ...
  if (error) {
    return res.status(HTTP.UNPROCESSABLE_ENTITY).json(joiErrorHandling(error));
  }
  ...
  try {
    ...
    return res.status(HTTP.UNPROCESSABLE_ENTITY).json(mongoErrorHandling(err));
  }
};
...
const update = async (req, res) => {
  ...
  if (!alert) {
    return res
      ...
      .json(commonErrorHandling("_id", "Alert not found"));
  }
